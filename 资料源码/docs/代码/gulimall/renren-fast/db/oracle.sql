...
CREATE
UNIQUE INDEX index_sys_user_username on sys_user(username);
...
CREATE
UNIQUE INDEX index_token on sys_user_token(token);
...
CREATE
UNIQUE INDEX index_param_key on sys_config(param_key);
...
CREATE
INDEX index_job_id on schedule_job_log(job_id);
...
CREATE
UNIQUE INDEX index_tb_user_username on tb_user(username);
